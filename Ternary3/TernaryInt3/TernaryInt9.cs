...
public struct TernaryInt9
{
    ...
    /// </summary>
    public static TernaryInt9 Parse(ReadOnlySpan<char> s) => new TernaryInt9(Parser.ToTrits16(s, 9));

    /// <summary>
    ///  Converts the <see cref="string"/> representation of a value to a <see cref="TernaryInt16"/> instance.
    /// </summary>
    public static TernaryInt9 Parse(string s) => Parse(s.AsSpan());
    ...
    /// Gets the nth trit (zero-based, low first)
    /// </summary>
    public Trit this[Index index] => Operations.GetTrit(trits, index, 9);

    /// <summary>
    /// Gets a range of trits (zero based, low first)
    /// </summary>
    public TernaryInt9 this[Range range] => new TernaryInt9(Operations.GetTrits(trits, range, 9));
